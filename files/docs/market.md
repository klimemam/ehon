...
**これは [`works/map-axes.md`](../works/map-axes.md) 軸A-3 が予測した通りのことが起きた例である。** 異なる分野（機構設計・市場設計）の事前分布を与えると、同じ結論に別の道から着く。しかも**用語が一つも共通していない**——CPCも陪審定理も出てこないのに、同じ場所に着いている。
...
| §1〜§7 §9 §12 §16 | 本リポジトリが扱っていなかった**新領域**。[`works/map.md`](../works/map.md) 第八部として追加 |
